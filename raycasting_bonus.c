...
/*   Updated: 2021/04/22 10:20:50 by hmesnard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_ver_line(t_d *d, t_rc *rc, int x)
{
	...
	while (y < d->i.y)
	{
		if (rc->dstart <= y && y < rc->dend)
		{
			...
			pixel_put(&d->i, x, y, shadow(shadow(rc->color,
						(((10 - rc->perp) / 10))), d->life / 50.0));
		}
		else if (rc->dstart > y)
			pixel_put(&d->i, x, y, shadow(d->ccl, d->life / 50.0));
		else
			pixel_put(&d->i, x, y, shadow(shadow(d->fcl,
						(y - (int)((11.0 / 20.0) * d->i.y))
						/ ((9.0 / 20.0) * d->i.y)), d->life / 50.0));
		y++;
	}
}
